...
export async function fetchLatestUserIds(count: number) {
  const res = await fetch(`${API_BASE_URL}/latestUsers/${count}`);
  if (!res.ok) throw new Error("Failed to fetch latest user IDs");
  return res.json();
}
